...
### References: 
### https://cran.r-project.org/web/packages/googleLanguageR/vignettes/nlp.html (Google NLP API),
### Data pre-cleaning: eBook, Mastering Social Media Mining in R (Ravindran, 2015), Cleaning the corpus
```{r}
# TODO. Here follows H4 testing. Measurement variables are: a. Message text, b. Polarity score
# 1. Retweets will be collected, from both sectors Banks and FinTech.
# 2. Polarity score will be calculated based on that tweets, with Google NLP API.
...
# Pre-cleaning unstructured data
ft_retweets_clean <- cleanTweets(fintech_data_retweets$text)
head (ft_retweets_clean)

# Structuring the unstructured
#ft_retweets_final <- ft_retweets_clean %>%
#  unique(ft_retweets_clean) %>% # Remove repetitive tweets
#  filter(n() == 10) # Minimum size of tweets for NLP analysis

# Remove repetitive tweets
ft_retweets_final=unique(ft_retweets_final)
head(ft_retweets_final)

# Google NLP API call
# nlp_result <- gl_nlp(ft_retweets_final)
