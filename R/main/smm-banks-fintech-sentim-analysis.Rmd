...
myAdjList <- list()
# Loops
for (rowIdx_i in 1:nrow(ba_resp_melt)) {
  print (rowIdx_i)
  
  for (rowIdx_j in 1:nrow(ba_resp_melt)) {
    if(ba_resp_melt$id[rowIdx_i] == ba_resp_melt$replyToSID[rowIdx_j]) {
      myAdjList$from <- ba_resp_melt$variable[rowIdx_i]
      myAdjList$to <- ba_resp_melt$variable[rowIdx_j]
      myAdjList$value <- ba_resp_melt$value[rowIdx_j]
    }
  }
  
  # Format example: "anger", "r_anger", "1"
