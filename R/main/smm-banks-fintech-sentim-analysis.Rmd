...
## H3 Testing: H3: Writing lengthy tweets has an impact to the number of retweets and likes to companies in the Swiss Fintech sector and Swiss banking sector.
### References:
...
# 3. Two MANOVA (2 DVs, 1 IV) regression models will be build, for both sectors Banks and FinTech.
...
# TODO: Validation for multivariate tests, e.g. mshapiro.test?
...
## Testing H5. H5: Companies in the Swiss FinTech experience more joy based on responses to corporate tweets than Swiss banking companies.
### References:  https://www.datacamp.com/community/tutorials/sentiment-analysis-R
...
# 2.B. SYUZHET: Call syuzhet multilanguage sentiment analysis method, on tidy text format (WORDS)
# WORDS does not work atm - invalid input
ft_data_tidy_syu <- get_nrc_sentiment(ft_data_tidy$word, language = "english")
# With PARALLELIZATION, cluster creating for 2 cores
cl <- makeCluster(2)
clusterExport(cl = cl, c("get_nrc_sentiment"))
ft_data_tidy_syu <- get_nrc_sentiment(ft_data_tidy$word, language = "english", cl=cl)
stopCluster(cl)
...
# 3.B. Plotting
# Plots, WORDS (tidytext format)
barplot(
  sort(colSums(prop.table(ft_data_tidy_syu[, 1:8]))), 
  horiz = TRUE, 
  cex.names = 0.7, 
  las = 1, 
  main = "Emotions in FinTech", xlab="Percentage"
)
...
### References:
### tidytext: https://www.datacamp.com/community/tutorials/sentiment-analysis-R
...
# 1. Prep data to tidy text format, undertake sentiment analysis based on individual words, use tidytext libs
undesirable_words <- c(" der ", " die ", " das ", "hey", "la", "da", "uh", "ah")
# Create tidy text format: Unnested, Unsummarized, -Undesirables, Stop and Short words
ba_data_tidy <- banks_data_retweets %>%
  unnest_tokens(word, text) %>% #Break the text into individual words
  filter(!word %in% undesirable_words) %>% #Remove undesirables
  filter(!nchar(word) < 3) %>% #Words like "ah" or "oo"
  anti_join(get_stopwords(language="de")) #Data provided by the tidytext package, Language = DE

glimpse(ba_data_tidy)

# Call syuzhet multilanguage sentiment analysis method, on tidy text format (WORDS)
ba_data_nrc_tidy <- get_nrc_sentiment(ba_data_tidy$word, language = "german")
# With PARALLELIZATION, cluster creating for 2 cores
cl <- makeCluster(2)
clusterExport(cl = cl, c("get_nrc_sentiment"))
ba_data_nrc_tidy <- get_nrc_sentiment(ba_data_tidy$word, language = "german", cl=cl)
stopCluster(cl)

# 2. Prep data to clean tweets (whole sentences), undertake sentiment analysis based on tweets
ba_retweets_clean <- cleanTweets(banks_data_retweets$text)
...
# Plots, WORDS (tidytext format)
barplot(
  sort(colSums(prop.table(ba_data_nrc_tidy[, 1:8]))), 
  horiz = TRUE, 
  cex.names = 0.7, 
  las = 1, 
  main = "Emotions in Banking", xlab="Percentage"
)
...
title("Banks Sector: Relationship Between Mood and Tweets")
